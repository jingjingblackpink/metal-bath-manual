...
> 入口二维码：`46二维码.bmp`（历次会话均未改动二维码图片及其指向链接）
...
## 七、最终线上状态（2026年9月9日核验；9月10日 PDF 更新后状态见第九节）
...
## 九、2026年9月10日会话：更新中英文说明书 PDF 至 20260910 版

> 沿用「遗留建议 2」做法：直接覆盖 `docs/B5_MAX_Metal_Bath_Manual_*.pdf` 同名文件并推送，站点文件名、下载链接、二维码均不变。
> **Commit：`cfe8a1a`（docs: 更新中英文说明书PDF至20260910版）、`6dac2f2`（docs: 版本戳时间更新为2026年9月10日）**

### 9.1 替换文件

| 站点文件（名未改） | 替换为本地新文件 | 大小 | 页数 |
|--------------------|------------------|------|------|
| `docs/B5_MAX_Metal_Bath_Manual_zh.pdf` | `B5_MAX_manual_zh_20260910.pdf` | 2,042,538 字节（≈1.94 MB） | 52 页 |
| `docs/B5_MAX_Metal_Bath_Manual_en.pdf` | `B5_MAX_manual_en_20260910.pdf` | 2,072,130 字节（≈1.97 MB） | 44 页 |

### 9.2 推送前核验（PyMuPDF 提取全文）

- 两个 PDF 均为 **B5 MAX** 正确型号（无误传 B5 PRO），关键数据齐全（50 组程序×10 步、12800 数据点、B5.MAX-V3.3.5、4~100°C、7 寸 DGUS）
- 与 20260908 版全文逐段比对，差异仅 1 处内容修正（中英文同步）：
  - 部件表「运动组件」→「模块组件」（英文 `Motion` → `Module`）
  - 「样品托盘部件 / 偏心运动部件」→「热盖 / 风扇」（英文 `Specimen tray / Eccentric` → `Heated Lid / Fan`）——修正了自涡旋仪说明书复制残留的错误部件名

### 9.3 页面同步更新

- `manuals.html`：说明书卡片大小文案 中文 `PDF 1.85 MB` → `PDF 1.94 MB`、英文 `PDF 1.87 MB` → `PDF 1.97 MB`
- `index.html`：右上角版本戳 `时间：2026年9月9日` → `时间：2026年9月10日`
- 仓库根《二维码使用说明.md》PDF 文件清单大小 `1.63MB / 1.59 MB`（旧残留值）→ `1.94 MB / 1.97 MB`
- `style.css` 未改动，`?v=20260908` 参数保持不变（本次未改 CSS，无缓存风险）

### 9.4 上线验证（GitHub Pages 重建后实测）

- 两个 PDF 均 `200 OK`，线上与本地新文件**逐字节一致**（2,042,538 / 2,072,130 字节，MD5 `e3f95fc304cc…` / `23f5ca9b6dd6…`）
